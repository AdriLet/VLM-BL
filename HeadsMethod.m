...
for j=2:N
    ...
    H_star=real(1.535*(H(j-1)-0.7)^(-2.715)+3.3);
    CE=real(0.0306*(H_star-3)^(-0.653));    
    ...
    dHdS=1/(theta(j-1)*1.535*(H(j-1)-0.7)^(-3.715)*(-2.715))*(CE-theta(j-1)*H_star/V(j-1)*dVds(j)-H_star*dThetadS);
        H(j)=real(H(j-1)+dS(j)*dHdS);
    theta(j)=real(theta(j-1)+dS(j)*dThetadS);
     R_theta=Vinf*theta(j)/nu;
    Cf(j)=real(0.246*10^(-0.678*H(j))*R_theta^(-0.268));
end
